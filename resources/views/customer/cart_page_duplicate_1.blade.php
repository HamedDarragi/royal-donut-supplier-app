                            ...
                            <div class="card" id="Supplier{{ $supplier->id }}">
                                @if ($carts->parent == null)
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if ($find_associate_rule)
                                            {{-- {{ dump('SS__1111--' . $supplier->id) }} --}}
                                            <a class="btn mo " data-toggle="modal"
                                                data-target="#myModal{{ $carts->id . '-' . $supplier->id . '-' . auth()->user()->id . '-' . $c_sup->delivery_company_id }}"
                                                name="{{ $carts->id . '-' . $supplier->id . '-' . auth()->user()->id . '-' . $c_sup->delivery_company_id }}"
                                                style="background-color: #ec607f"
                                                id="fixedbutton">{{ trans('french.Confirm') }}
                                                <i class="fa fa-arrow-right"></i></a>
                                            <!-- The Modal -->
                                            <div class="modal fade"
                                                id="myModal{{ $carts->id . '-' . $supplier->id . '-' . auth()->user()->id . '-' . $c_sup->delivery_company_id }}"
                                                name="{{ $carts->id . '-' . $supplier->id . '-' . auth()->user()->id . '-' . $c_sup->delivery_company_id }}">
                                ...
                                @else
                                    ...
                                    @else
                                        @if ($find_associate_rule_2 == null && $find_associate_rule == null)
